...
# DRAW LINES IN CORNERS

for i in range(len(corners)):       #loop through all corners
    for j in range(i+1,len(corners)):  #loop through all other corner that we already looped
        corner1 = tuple(corners[i][0])
        corner2 = tuple(corners[j][0])
        color = tuple(map(lambda x: int(x), np.random.randint(0, 255, size=3)))    #numpy has 64 or 32bit integer but we need regular 8 bit so we map it with a funtion, finally convert into tuple
